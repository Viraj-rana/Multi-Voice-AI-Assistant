...
#correct version of the code starts from here 
# use python cli.py to run CLI version both version are available
...
# Define output directory early so all functions can use it
project_dir = os.path.dirname(os.path.abspath(__file__))
output_dir = os.path.join(project_dir, 'outputs')
os.makedirs(output_dir, exist_ok=True)
...
def openai_text_to_speech(prompt, output_path):
    ...
    if file_extension == 'wav':
        ...
        try:
            ...
            print("Audio generated successfully with OpenAI.")
            # Removed play_audio() call to avoid double playback
        except requests.HTTPError as e:
            print(f"Error during OpenAI TTS: {e}")
...
def analyze_image(image_path, question_prompt):
    ...
    if MODEL_PROVIDER == 'ollama':
        ...
        if not XAI_API_KEY:
            return {"choices": [{"message": {"content": "XAI_API_KEY not set for image analysis."}}]}
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {XAI_API_KEY}"
        }
...
def analyze_image_with_openai(encoded_image, question_prompt):
    if not OPENAI_API_KEY:
        return {"choices": [{"message": {"content": "OPENAI_API_KEY not set for image analysis."}}]}
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {OPENAI_API_KEY}"
    }
...
def generate_speech(text, temp_audio_path):
    if TTS_PROVIDER == 'openai':
        if not OPENAI_API_KEY:
            print("OpenAI API key missing, cannot generate speech.")
            return
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {OPENAI_API_KEY}"
